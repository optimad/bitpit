...
uint32_t Class_Map::mapX(double const & X){
	return (double(max_length)/L * (double(X) - X0));
}
uint32_t Class_Map::mapY(double const & Y){
	return (double(max_length)/L * (double(Y) - Y0));
}
uint32_t Class_Map::mapZ(double const & Z){
	return (double(max_length)/L * (double(Z) - Z0));
}
...
void Class_Map::mapNodesIntersection(uint32_t (*nodes)[DIM], vector<vector<double> > & mapnodes){
	vector<double> orig;
	orig.push_back(X0);
	orig.push_back(Y0);
	if (DIM == 3)
