...
//}
//
//void Class_Para_Tree::computePartition(uint32_t* partition, uint8_t & level_) {
////	level = uint8_t(min(int(level), MAX_LEVEL));
//	uint8_t level = uint8_t(min(int(max(int(max_depth) - int(level_), int(1))) , MAX_LEVEL));
//	uint32_t partition_temp[nproc];
//	uint8_t boundary_proc[nproc-1], dimcomm, glbdimcomm[nproc], indcomm, glbindcomm[nproc];
//	uint32_t division_result = 0;
//	uint32_t remind = 0;
//	uint32_t Dh = uint32_t(pow(double(2),double(MAX_LEVEL-level)));
...
//}
//
//void Class_Para_Tree::loadBalance(uint8_t & level){
//
//	//Write info on log
//	writeLog("---------------------------------------------");
//	writeLog(" LOAD BALANCE ");
...
//}
//
//void Class_Para_Tree::updateAfterCoarse(u32vector & mapidx){
//	if(serial){
//		updateAdapt();
//	}
//	else{
...
//
//
//double Class_Para_Tree::getSize(Class_Intersection* const inter) {
//	return trans.mapSize(inter->getSize());
//}
//
//double Class_Para_Tree::getArea(Class_Intersection* const inter) {
//	return trans.mapArea(inter->getArea());
//}
//
//void Class_Para_Tree::getCenter(Class_Intersection* inter,
//								vector<double>& center) {
...
//}
//
//void Class_Para_Tree::getNodes(Class_Intersection* const inter,
//								dvector2D & nodes) {
//	uint32_t (*nodes_)[DIM] = inter->getNodes();
//	trans.mapNodesIntersection(nodes_, nodes);
//	delete [] nodes_;
//	nodes_ = NULL;
//}
//
//void Class_Para_Tree::getNormal(Class_Intersection* const inter,
//								dvector & normal) {
//}
//
//// =================================================================================== //
