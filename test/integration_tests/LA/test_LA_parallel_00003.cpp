...
int subtest_001(int rank, int nProcs)
{
    ...
    // Restore system
    SystemSolver restoredSystem(MPI_COMM_WORLD, "./dump_A.txt", "", false);
    ...
    // Initialize RHS
    double *rhs = restoredSystem.getRHSRawPtr();
    for (int i = 0; i < nCols; ++i) {
        rhs[i] = 1.;
    }
    restoredSystem.restoreRHSRawPtr(rhs);

    // Initialize solution
    double *initialSolution = restoredSystem.getSolutionRawPtr();
    for (int i = 0; i < nRows; ++i) {
        initialSolution[i] = 0;
    }
    restoredSystem.restoreSolutionRawPtr(initialSolution);
