...
Code developed in <B>%patchman</B> should follow the coding styles described here.  Any deviations from this style
guide will result in severe berating and other verbal abuse.

\section directory_structure patchman Directory Structure
<B>%patchman</B> source code is organized in the following directory structure: \n
 - doc: documentation is put here, along with the input file for Doxygen.  Most <B>%patchman</B> documentation is doxygen-processed;
 - examples: examples of <B>%patchman</B> usage, both small and large.  These programs are not meant to be used as unit tests, but
     rather as further documentation on <B>%patchman</B> usage;
 - src : source code;
 - test: all unit test programs should go below this directory.

If you're designing a new class or other code for <B>%patchman</B> and are not sure where to put it, try to find something similar
and put it there.  Otherwise, email the <B>%patchman</B> email list for pointers.  <em> In general, you should not need to create new
subdirectories in the <B>%patchman</B> source code.</em>
...
Class methods names should be in the snake_case style, e.g. Patch::get_id()
or Interface::get_area().
...
m_memberVariable it is necessary to define void set_member_variable(int newval)
and int get_member_variable() const.
...
Any names introduced into the top-level <B>%patchman</B> namespace should be
sufficiently unique to avoid conflicts with other code.  If you must
introduce a class to the top-level <B>%patchman</B> namespace, don't choose
...
headers, introduce names into the global rather than <B>%patchman</B> namespace.
...
Use a space after (most) keywords.  The
...
Each class should be fully commented with a \\class comment block, formatted like those in the <B>%patchman</B> core classes. <B>The arguments of the class constructor are very important</B>,
...
See the <B>%patchman</B> classes for examples of how to format these comments.  As a rule of thumb, your code should run through
