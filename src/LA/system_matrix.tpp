...
#ifndef __BITPIT_SYSTEM_MATRIX_TPP__
#define __BITPIT_SYSTEM_MATRIX_TPP__
...
*/
template<typename T>
SparseMatrix<T>::SparseMatrix(MPI_Comm communicator)
...
*/
template<typename T>
SparseMatrix<T>::SparseMatrix(long nRows, long nCols, long nNZ)
...
*/
template<typename T>
SparseMatrix<T>::SparseMatrix(MPI_Comm communicator, bool partitioned, long nRows, long nCols, long nNZ)
...
*/
template<typename T>
void SparseMatrix<T>::initialize(bool partitioned, long nRows, long nCols, long nNZ)
{
    _initialize(partitioned, nRows, nCols, nNZ);
}
#endif
...
*/
template<typename T>
void SparseMatrix<T>::initialize(long nRows, long nCols, long nNZ)
{
#if BITPIT_ENABLE_MPI==1
...
*/
template<typename T>
void SparseMatrix<T>::_initialize(bool partitioned, long nRows, long nCols, long nNZ)
{
...
*/
template<typename T>
void SparseMatrix<T>::_initialize(long nRows, long nCols, long nNZ)
{
    assert(nRows >= 0);
    assert(nCols >= 0);
    assert(nNZ >= 0);
...
*/
template<typename T>
void SparseMatrix<T>::clear(bool release)
{
    if (release) {
        m_pattern.clear();
        m_values.clear();
...
*/
template<typename T>
bool SparseMatrix<T>::isAssembled() const
{
    return m_assembled;
}
...
*/
template<typename T>
long SparseMatrix<T>::countMissingRows() const
{
...
*/
template<typename T>
long SparseMatrix<T>::countAddedRows() const
{
    return (m_lastRow + 1);
}
...
*/
template<typename T>
long SparseMatrix<T>::getRowCount() const
{
    return m_nRows;
}
...
*/
template<typename T>
long SparseMatrix<T>::getColCount() const
{
    return m_nCols;
}
...
*/
template<typename T>
long SparseMatrix<T>::getNZCount() const
{
    return m_nNZ;
}
...
*/
template<typename T>
long SparseMatrix<T>::getRowNZCount(long row) const
{
...
*/
template<typename T>
long SparseMatrix<T>::getMaxRowNZCount() const
{
    return m_maxRowNZ;
}
...
*/
template<typename T>
bool SparseMatrix<T>::isPartitioned() const
{
...
*/
template<typename T>
const MPI_Comm & SparseMatrix<T>::getCommunicator() const
{
...
*/
template<typename T>
void SparseMatrix<T>::setCommunicator(MPI_Comm communicator)
{
...
*/
template<typename T>
void SparseMatrix<T>::freeCommunicator()
{
...
*/
template<typename T>
long SparseMatrix<T>::getRowGlobalCount() const
{
...
*/
template<typename T>
long SparseMatrix<T>::getRowGlobalOffset() const
{
...
*/
template<typename T>
long SparseMatrix<T>::getColGlobalCount() const
{
...
*/
template<typename T>
long SparseMatrix<T>::getColGlobalOffset() const
{
...
*/
template<typename T>
long SparseMatrix<T>::getNZGlobalCount() const
{
    return m_global_nNZ;
}
...
*/
template<typename T>
long SparseMatrix<T>::getMaxRowNZGlobalCount() const
{
...
*/
template<typename T>
std::vector<long> SparseMatrix<T>::extractLocalGlobalRows() const
{
...
*/
template<typename T>
std::vector<long> SparseMatrix<T>::extractGhostGlobalRows() const
{
...
*/
template<typename T>
std::vector<long> SparseMatrix<T>::extractLocalGlobalCols() const
{
...
*/
template<typename T>
std::vector<long> SparseMatrix<T>::extractGhostGlobalCols() const
{
    long firstGlobalCol = m_global_colOffset;
    long lastGlobalCol  = m_global_colOffset + m_nCols - 1;
...
*/
template<typename T>
void SparseMatrix<T>::addRow(const std::vector<long> &rowPattern, const std::vector<T> &rowValues)
{
...
*/
template<typename T>
void SparseMatrix<T>::addRow(long nRowNZ, const long *rowPattern, const T *rowValues)
{
    if (countMissingRows() == 0) {
...
*/
template<typename T>
ConstProxyVector<long> SparseMatrix<T>::getRowPattern(long row) const
{
...
*/
template<typename T>
void SparseMatrix<T>::getRowPattern(long row, ConstProxyVector<long> *pattern) const
{
...
*/
template<typename T>
ConstProxyVector<T> SparseMatrix<T>::getRowValues(long row) const
{
    ConstProxyVector<T> values;
...
*/
template<typename T>
void SparseMatrix<T>::getRowValues(long row, ConstProxyVector<T> *values) const
{
    const std::size_t *rowExtent = m_pattern.indices(row);

    const T *rowValues = m_values.data() + rowExtent[0];
...
*/
template<typename T>
std::unique_ptr<SparseMatrix<T>> SparseMatrix<T>::computeTranspose() const
{
    ...
    for (long row = 0; row < m_nRows; ++row) {
        ...
        const T *rowValues = m_values.data() + rowExtents[row];
        for (std::size_t k = 0; k < rowPatternSize; ++k) {
            ...
            *transposeRowLastPattern = row;
            ...
            T *transposeRowLastValue = transpose->m_values.data() + transposeRowLastIndex;
