 ...
 */
uint32_t	ClassOctant::getX() const{return m_x;};

/*! Get the coordinates of an octant, i.e. the coordinates of its node 0.
 * \return Coordinate Y of node 0.
 */
uint32_t	ClassOctant::getY() const{return m_y;};

/*! Get the coordinates of an octant, i.e. the coordinates of its node 0.
 * \return Coordinate Z of node 0.
 */
uint32_t	ClassOctant::getZ() const{return m_z;};
 ...
 */
uint8_t		ClassOctant::getLevel() const{return m_level;};

/*! Get the refinement marker of an octant.
 * \return Marker of octant.
 */
int8_t		ClassOctant::getMarker() const{return m_marker;};
 ...
 * \return true if the iface face is a process boundary face.
 */
bool		ClassOctant::getPbound() const{
 ...
 */
darray3	ClassOctant::getCenter(int8_t maxlevel) const{
	double	dh;
 ...
 */
darray3	ClassOctant::getFaceCenter(uint8_t iface, int8_t maxlevel) const{
	double	dh_2;
 ...
 */
darray3	ClassOctant::getEdgeCenter(uint8_t iedge, int8_t maxlevel) const{
	double	dh_2;
 ...
 */
void		ClassOctant::getNodes(u32arr3vector & nodes, int8_t maxlevel) const{
	uint8_t		i, cx, cy, cz;
	uint32_t	dh;
	uint8_t nn = 1<<m_dim;
...
void		ClassOctant::getNormal(uint8_t & iface,
		i8array3 & normal, int8_t (&normals)[6][3], int8_t maxlevel) const{
	uint8_t		i;
	for (i = 0; i < 3; i++){
...
uint64_t 		ClassOctant::computeNodeHalfSizeMorton(uint8_t inode, 		// Computes Morton index (without level) of "n=sizehf" half-size (or same size if level=maxlevel)
	...
	if (m_info[iface[0]] || m_info[iface[1]] || m_info[iface[2]]){
		sizehf = 0;
