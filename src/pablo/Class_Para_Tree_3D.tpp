...
template<>
class Class_Para_Tree<3> : public Class_Para_Tree_Base {
	// ------------------------------------------------------------------------------- //
	// TYPEDEFS ----------------------------------------------------------------------- //
public:
...
public:
	...
	Class_Para_Tree(string logfile="PABLO.log") : log(logfile){
		...
		log.writeLog(" Dimension		:	" + to_string(static_cast<unsigned long long>(3)));
		log.writeLog(" Max allowed level	:	" + to_string(static_cast<unsigned long long>(MAX_LEVEL_3D)));
		log.writeLog("---------------------------------------------");
		log.writeLog(" ");
#if NOMPI==0
		MPI_Barrier(comm);
...
public:
	 ...
	 */
	int8_t getMarker(Class_Octant<3>* oct){								// Get refinement/coarsening marker for idx-th octant
...
public:
	...
	void getBoundingBox(dvector & P0, dvector & P1){
		...
		P0 = getNode(0, 0);
		P1 = getNode(nocts-1, global3D.nnodes-1);
		for (uint32_t idx=0; idx<global3D.nnodes; idx++){
	...
	void getBoundingBox(array<double, 3> & P0, array<double, 3> & P1){
		...
		uint32_t 	nocts = getNumOctants();
		cnode0 = getNode(0, 0);
		cnode1 = getNode(nocts-1, global3D.nnodes-1);
		for (uint8_t i=0; i<3; i++){
			P0[i] = cnode0[i];
			P1[i] = cnode1[i];
		}
			...
			if(headSize != 0){
				for(int p = firstPredecessor; p >= 0; --p){
					if(headSize < partition[p]){
						...
						int buffSize = nofElementsFromSuccessiveToPrevious * (int)ceil((double)global3D.octantBytes / (double)(CHAR_BIT/8));
			...
			if(tailSize != 0){
				for(int p = firstSuccessor; p < nproc; ++p){
					if(tailSize < partition[p]){
						...
						int buffSize = nofElementsFromPreviousToSuccessive * (int)ceil((double)global3D.octantBytes / (double)(CHAR_BIT/8));
						sendBuffers[p] = Class_Comm_Buffer(buffSize,'a',comm);
						int pos = 0;
	...
	bool adapt_mapidx(){  					//call refine and coarse on the local tree
		...
		if(serial){
			...
			if (octree.getNumOctants() > nocts)
				localDone = true;
			nocts = octree.getNumOctants();
			log.writeLog(" Number of octants after Refine	:	" + to_string(static_cast<unsigned long long>(nocts)));
			updateAdapt();
