...
public:
	 ...
	 */
	vector<double>	getFaceCenter(uint8_t iface) const;
	 ...
	 */
	vector<double>	getEdgeCenter(uint8_t iedge) const;
	 ...
	 */
	void		getNodes(u32vector2D & nodes) const;
	 ...
	 */
	void		getNode(u32vector & node, uint8_t inode) const;

	/*! Get the coordinates of a nodes of an octant in logical domain.
	 * \param[in] inode Local index of the node
	 * \param[out] node dim-vector with the logical coordinates of the node of the octant.
	 */
	u32vector		getNode(uint8_t inode) const;
	 ...
	 */
	void		getNormal(uint8_t & iface, vector<int8_t> & normal) const;
...
private:
	...
	classOctant	buildLastDesc();

	classOctant	buildFather();

	/** Builds children of octant.
	 *   \return Ordered (by Z-index) vector of children[nchildren] (info update)
	 */
	vector< classOctant >	buildChildren();

	vector<uint64_t> 		computeHalfSizeMorton(uint8_t iface, uint32_t & sizehf);
	...
	vector<uint64_t>		computeEdgeVirtualMorton(uint8_t iedge, const uint8_t & maxdepth,
			uint32_t & sizeneigh, uint8_t balance_codim);

	uint64_t 		computeNodeHalfSizeMorton(uint8_t inode, uint32_t & sizehf);

	uint64_t 		computeNodeMinSizeMorton(uint8_t inode, const uint8_t & maxdepth, uint32_t & sizehf);

	uint64_t 		computeNodeVirtualMorton(uint8_t inode, const uint8_t & maxdepth, uint32_t & sizeneigh);
