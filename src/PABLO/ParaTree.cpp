...
/*! Default constructor
 */
ParaTree::LoadBalanceRanges::LoadBalanceRanges() : sendAction(ACTION_UNDEFINED), recvAction(ACTION_UNDEFINED) {}
 ...
 */
ParaTree::LoadBalanceRanges::LoadBalanceRanges(bool serial, const ExchangeRanges& _sendRanges,
 ...
 * communications.
 */
void ParaTree::_initializeCommunicator(MPI_Comm communicator) {
 ...
 */
void ParaTree::_initializeSerialCommunicator() {
    m_nproc = 1;
    m_rank = 0;
}
#endif
 ...
 */
void ParaTree::_initializePartitions() {
 ...
 * \param[in] logfile The file name for the log of this object. PABLO.log is the default value.
 */
void ParaTree::_initialize(uint8_t dim, const std::string& logfile) {
 ...
 * \param[in] comm The MPI communicator used by the parallel octree. MPI_COMM_WORLD is the default value.
 */
void ParaTree::initialize(const std::string& logfile, MPI_Comm comm) {
#else
void ParaTree::initialize(const std::string& logfile) {
#endif
#if BITPIT_ENABLE_MPI == 1
 ...
 * \param[in] logfile The file name for the log of this object. PABLO.log is the default value.
 */
void ParaTree::reinitialize(uint8_t dim, const std::string& logfile) {
    // Initialized the tree
    if(dim < 2 || dim > 3) {
        throw std::runtime_error("Invalid value for the dimension");
    }
...
/*! Initialize the logger
 */
void ParaTree::initializeLogger(const std::string& logfile) {
...
/*! Reset the octree
 */
void ParaTree::reset(bool createRoot) {
    m_tol = 1.0e-14;
    m_serial = true;
    m_errorFlag = 0;
...
void ParaTree::printHeader() {
    ...
    (*m_log) << " Max allowed level	:	" +
                    to_string(static_cast<unsigned long long>(m_treeConstants->maxLevel))
             << endl;
    (*m_log) << " Number of octants	:	" + to_string(static_cast<unsigned long long>(m_globalNumOctants))
             << endl;
    (*m_log) << "---------------------------------------------" << endl;
    (*m_log) << " " << endl;
}
 ...
 */
uint64_t ParaTree::getGlobalNumOctants() const {
 ...
 */
bool ParaTree::getParallel() const {
    return (!m_serial);
};
 ...
 */
Logger& ParaTree::getLog() {
    return (*m_log);
}

/*!Get the last operation perforfmed by the octree (initialization, adapt (mapped or unmapped), loadbalance (first or
 * not). \return Last operation performed by the octree.
 */
ParaTree::Operation ParaTree::getLastOperation() const {
    return m_lastOp;
}
 ...
 * communications.
 */
void ParaTree::setComm(MPI_Comm communicator) {
 ...
 * will be freed.
 */
void ParaTree::replaceComm(MPI_Comm communicator, MPI_Comm* previousCommunicator) {
 ...
 */
MPI_Comm ParaTree::getComm() const {
    return m_comm;
};
#endif
 ...
 * global index of the last existing octant in each process.
 */
const std::vector<uint64_t>& ParaTree::getPartitionRangeGlobalIdx() const {
 ...
 * Morton number of the first octant on each process.
 */
const std::vector<uint64_t>& ParaTree::getPartitionFirstDesc() const {
 ...
 * Morton number of the last possible octant on each process.
 */
const std::vector<uint64_t>& ParaTree::getPartitionLastDesc() const {
 ...
 */
darray3 ParaTree::getOrigin() const {
 ...
 */
int ParaTree::getMaxLevel() const {
 ...
 */
uint32_t ParaTree::getMaxLength() const {
 ...
 */
uint8_t ParaTree::getNnodes() const {
 ...
 */
uint8_t ParaTree::getNfaces() const {
 ...
 */
uint8_t ParaTree::getNedges() const {
 ...
 */
uint8_t ParaTree::getNchildren() const {
 ...
 */
uint8_t ParaTree::getNnodesperface() const {
    return m_treeConstants->nNodesPerFace;
}

/*! Get the components (in logical domain) of the 6 normals to the faces of an octant (for the 2D case consider only the
 * first 4) \param[out] normals Normals array[6][3] to the faces of an octant.
 */
void ParaTree::getNormals(int8_t normals[6][3]) const {
    for(int i = 0; i < 6; i++) {
        for(int j = 0; j < 3; j++) {
 ...
 */
void ParaTree::getOppface(uint8_t oppface[6]) const {
    for(int j = 0; j < 6; j++) {
 ...
 */
void ParaTree::getFacenode(uint8_t facenode[6][4]) const {
    for(int i = 0; i < 6; i++) {
        for(int j = 0; j < 4; j++) {
 ...
 */
void ParaTree::getNodeface(uint8_t nodeface[8][3]) const {
    for(int i = 0; i < 8; i++) {
        for(int j = 0; j < 3; j++) {
 ...
 */
void ParaTree::getEdgeface(uint8_t edgeface[12][2]) const {
    for(int i = 0; i < 12; i++) {
        for(int j = 0; j < 2; j++) {
 ...
 */
void ParaTree::getNodecoeffs(int8_t nodecoeffs[8][3]) const {
    for(int i = 0; i < 8; i++) {
        nodecoeffs[i][2] = 0;
 ...
 */
void ParaTree::getEdgecoeffs(int8_t edgecoeffs[12][3]) const {
    for(int i = 0; i < 12; i++) {
        for(int j = 0; j < 3; j++) {
...
/*! Get the components (in logical domain) of the 6 normals to the faces of an octant (for the 2D case consider only the
 * first 4) \return Pointer to normals array[6][3] to the faces of an octant.
 */
const int8_t (*ParaTree::getNormals() const)[3] {
 ...
 * face of an octant neighbour through the i-th face of the current octant).
 */
const uint8_t* ParaTree::getOppface() const {
 ...
 * of the i-th face of an octant.
 */
const uint8_t (*ParaTree::getFacenode() const)[4] {
 ...
 * sharing the i-th node of an octant.
 */
const uint8_t (*ParaTree::getNodeface() const)[3] {
 ...
 * faces sharing the i-th edge of an octant.
 */
const uint8_t (*ParaTree::getEdgeface() const)[2] {
 ...
 */
const int8_t (*ParaTree::getNodecoeffs() const)[3] {
 ...
 */
const int8_t (*ParaTree::getEdgecoeffs() const)[3] {
 ...
 */
bvector ParaTree::getPeriodic() const {
    return m_periodic;
};
 ...
 */
bool ParaTree::getPeriodic(uint8_t i) const {
    return m_periodic[i];
};

/*!Get the tolerance used in geometric operations.
 */
double ParaTree::getTol() const {
    return m_tol;
};
 ...
 */
void ParaTree::setPeriodic(uint8_t i) {
    m_periodic[i] = true;
 ...
 */
void ParaTree::setTol(double tol) {
    m_tol = tol;
};
 ...
 */
darray3 ParaTree::getCoordinates(uint32_t idx) const {
 ...
 */
double ParaTree::getX(uint32_t idx) const {
 ...
 */
double ParaTree::getY(uint32_t idx) const {
 ...
 */
double ParaTree::getZ(uint32_t idx) const {
 ...
 */
double ParaTree::getSize(uint32_t idx) const {
 ...
 */
double ParaTree::getArea(uint32_t idx) const {
 ...
 */
double ParaTree::getVolume(uint32_t idx) const {
 ...
 */
void ParaTree::getCenter(uint32_t idx, darray3& centerCoords) const {
 ...
 */
darray3 ParaTree::getCenter(uint32_t idx) const {
 ...
 */
darray3 ParaTree::getFaceCenter(uint32_t idx, uint8_t face) const {
    darray3 centerCoords = {{0., 0., 0.}};
 ...
 */
void ParaTree::getFaceCenter(uint32_t idx, uint8_t face, darray3& centerCoords) const {
    darray3 centerCoords_ = m_octree.m_octants[idx].getLogicalFaceCenter(face);
    m_trans.mapCenter(centerCoords_, centerCoords);
}
 ...
 */
darray3 ParaTree::getNode(uint32_t idx, uint8_t node) const {
 ...
 */
void ParaTree::getNode(uint32_t idx, uint8_t node, darray3& nodeCoords) const {
    u32array3 nodeCoords_ = m_octree.m_octants[idx].getLogicalNode(node);
    m_trans.mapNode(nodeCoords_, nodeCoords);
}
 ...
 */
void ParaTree::getNodes(uint32_t idx, darr3vector& nodesCoords) const {
 ...
 */
darr3vector ParaTree::getNodes(uint32_t idx) const {
 ...
 */
void ParaTree::getNormal(uint32_t idx, uint8_t face, darray3& normal) const {
    i8array3 normal_;
 ...
 */
darray3 ParaTree::getNormal(uint32_t idx, uint8_t face) const {
 ...
 */
int8_t ParaTree::getMarker(uint32_t idx) const {
 ...
 */
int8_t ParaTree::getPreMarker(uint32_t idx) {
 ...
 */
uint8_t ParaTree::getLevel(uint32_t idx) const {
 ...
 */
uint64_t ParaTree::getMorton(uint32_t idx) const {
 ...
 */
uint64_t ParaTree::computeNodePersistentKey(uint32_t idx, uint8_t node) const {
 ...
 */
bool ParaTree::getBalance(uint32_t idx) const {
 ...
 */
bool ParaTree::getBound(uint32_t idx, uint8_t face) const {
 ...
 */
bool ParaTree::getBound(uint32_t idx) const {
 ...
 */
bool ParaTree::getPbound(uint32_t idx, uint8_t face) const {
 ...
 */
bool ParaTree::getPbound(uint32_t idx) const {
 ...
 */
bool ParaTree::getIsNewR(uint32_t idx) const {
 ...
 * \return Is octant new?
 */
bool ParaTree::getIsNewC(uint32_t idx) const {
 ...
 */
uint64_t ParaTree::getGlobalIdx(uint32_t idx) const {
    if(m_rank) {
 ...
 * \param[in] rank Rank of the process owning the octant as a local one. getOwnerRank can be used to get the rank of the
 * process owning the octant locally. \return Local index of octant.
 */
uint32_t ParaTree::getLocalIdx(uint64_t gidx, int rank) const {
    if(rank) {
 ...
 */
void ParaTree::getLocalIdx(uint64_t gidx, uint32_t& lidx, int& rank) const {
 ...
 */
uint32_t ParaTree::getLocalIdx(uint64_t gidx) const {
    if(m_rank) {
 ...
 */
uint32_t ParaTree::getGhostLocalIdx(uint64_t gidx) const {
    uint32_t index;
 ...
 */
uint64_t ParaTree::getGhostGlobalIdx(uint32_t idx) const {
 ...
 * current process.
 */
bool ParaTree::isInternal(uint64_t gidx) const {
 ...
 */
bitset<72> ParaTree::getPersistentIdx(uint32_t idx) const {
 ...
 */
void ParaTree::setMarker(uint32_t idx, int8_t marker) {
    if(m_lastOp == OP_PRE_ADAPT) {
 ...
 */
void ParaTree::setBalance(uint32_t idx, bool balance) {
    if(m_lastOp == OP_PRE_ADAPT) {
        throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
    }
 ...
 */
darray3 ParaTree::getCoordinates(const Octant* oct) const {
 ...
 * \return Coordinate X of node 0.
 */
double ParaTree::getX(const Octant* oct) const {
 ...
 * \return Coordinate Y of node 0.
 */
double ParaTree::getY(const Octant* oct) const {
 ...
 * \return Coordinate Z of node 0.
 */
double ParaTree::getZ(const Octant* oct) const {
 ...
 * \return Size of octant.
 */
double ParaTree::getSize(const Octant* oct) const {
 ...
 * \return Area of octant.
 */
double ParaTree::getArea(const Octant* oct) const {
 ...
 * \return Volume of octant.
 */
double ParaTree::getVolume(const Octant* oct) const {
 ...
 * \param[out] centerCoords Coordinates of the center of octant.
 */
void ParaTree::getCenter(const Octant* oct, darray3& centerCoords) const {
 ...
 * \return Coordinates of the center of octant.
 */
darray3 ParaTree::getCenter(const Octant* oct) const {
    darray3 centerCoords = {{0., 0., 0.}};
    darray3 centerCoords_ = oct->getLogicalCenter();
    m_trans.mapCenter(centerCoords_, centerCoords);
    return centerCoords;
 ...
 * \return Coordinates of the center of the face-th face af octant.
 */
darray3 ParaTree::getFaceCenter(const Octant* oct, uint8_t face) const {
    darray3 centerCoords = {{0., 0., 0.}};
 ...
 * \param[out] centerCoords Coordinates of the center of the face-th face af octant.
 */
void ParaTree::getFaceCenter(const Octant* oct, uint8_t face, darray3& centerCoords) const {
    darray3 centerCoords_ = oct->getLogicalFaceCenter(face);
    m_trans.mapCenter(centerCoords_, centerCoords);
}
 ...
 * \return Coordinates of the node-th node of octant.
 */
darray3 ParaTree::getNode(const Octant* oct, uint8_t node) const {
    darray3 nodeCoords = {{0., 0., 0.}};
 ...
 * \param[out] nodeCoords Coordinates of the node-th node of octant.
 */
void ParaTree::getNode(const Octant* oct, uint8_t node, darray3& nodeCoords) const {
    u32array3 nodeCoords_ = oct->getLogicalNode(node);
    m_trans.mapNode(nodeCoords_, nodeCoords);
}
 ...
 * \param[out] nodes Coordinates of the nodes of octant.
 */
void ParaTree::getNodes(const Octant* oct, darr3vector& nodes) const {
    u32arr3vector nodesCoords_;
 ...
 * \return Coordinates of the nodes of octant.
 */
darr3vector ParaTree::getNodes(const Octant* oct) const {
    darr3vector nodes;
    u32arr3vector nodesCoords_;
    oct->getLogicalNodes(nodesCoords_);
    m_trans.mapNodes(nodesCoords_, nodes);
 ...
 * \param[out] normal Coordinates of the normal of face.
 */
void ParaTree::getNormal(const Octant* oct, uint8_t face, darray3& normal) const {
    i8array3 normal_;
 ...
 * \return Normal of the face.
 */
darray3 ParaTree::getNormal(const Octant* oct, uint8_t face) const {
    darray3 normal = {{0., 0., 0.}};
    i8array3 normal_ = {{0, 0, 0}};
    oct->getNormal(face, normal_, m_treeConstants->normals);
    m_trans.mapNormals(normal_, normal);
 ...
 * \return Marker of octant.
 */
int8_t ParaTree::getMarker(const Octant* oct) const {
 ...
 * NOTE: if a last operation is not preadapt, it calls preadapt method.
 */
int8_t ParaTree::getPreMarker(Octant* oct) {
    if(m_lastOp != OP_PRE_ADAPT) {
        throw std::runtime_error("Last operation different from preadapt, unable to call getPreMarker function");
    }
 ...
 * \return Level of octant.
 */
uint8_t ParaTree::getLevel(const Octant* oct) const {
    return oct->getLevel();
};
 ...
 * \return morton Morton index of the octant.
 */
uint64_t ParaTree::getMorton(const Octant* oct) const {
 ...
 */
uint64_t ParaTree::getLastDescMorton(const Octant* oct) const {
 ...
 * \return persistent XYZ key of the node.
 */
uint64_t ParaTree::computeNodePersistentKey(const Octant* oct, uint8_t node) const {
 ...
 * \return Has octant to be balanced?
 */
bool ParaTree::getBalance(const Octant* oct) const {
 ...
 * \return Is the face a boundary face?
 */
bool ParaTree::getBound(const Octant* oct, uint8_t face) const {
 ...
 * \return Is the octant a boundary octant?
 */
bool ParaTree::getBound(const Octant* oct) const {
    return oct->getBound();
}
 ...
 * \return Is the face a partition boundary face?
 */
bool ParaTree::getPbound(const Octant* oct, uint8_t face) const {
 ...
 * \return Is the octant a partition boundary octant?
 */
bool ParaTree::getPbound(const Octant* oct) const {
 ...
 * \return Is octant new?
 */
bool ParaTree::getIsNewR(const Octant* oct) const {
 ...
 * \return Is octant new?
 */
bool ParaTree::getIsNewC(const Octant* oct) const {
 ...
 * \return Local index of octant.
 */
uint32_t ParaTree::getIdx(const Octant* oct) const {
#if BITPIT_ENABLE_MPI == 1
    if(getIsGhost(oct)) {
 ...
 * \return Global index of octant.
 */
uint64_t ParaTree::getGlobalIdx(const Octant* oct) const {
 ...
 * i.e. a bitset composed by Morton index and level of octant.
 */
bitset<72> ParaTree::getPersistentIdx(const Octant* oct) const {
 ...
 * \param[in] marker Refinement marker of octant (n=n refinement in adapt, -n=n coarsening in adapt, default=0).
 */
void ParaTree::setMarker(Octant* oct, int8_t marker) {
    if(m_lastOp == OP_PRE_ADAPT) {
        throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
    }
 ...
 * \param[in] balance Has octant to be 2:1 balanced in adapting procedure?
 */
void ParaTree::setBalance(Octant* oct, bool balance) {
    if(m_lastOp == OP_PRE_ADAPT) {
        throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
    }
 ...
 */
uint64_t ParaTree::getStatus() const {
    return m_status;
}
 ...
 */
uint32_t ParaTree::getNumOctants() const {
 ...
 */
uint32_t ParaTree::getNumGhosts() const {
 ...
 */
uint32_t ParaTree::getNumNodes() const {
 ...
 */
uint8_t ParaTree::getLocalMaxDepth() const {
 ...
 */
double ParaTree::getLocalMinSize() const {
 ...
 */
double ParaTree::getLocalMaxSize() const {
 ...
 */
uint8_t ParaTree::getBalanceCodimension() const {
 ...
 * \return Morton index of the last finest descendant of the target octant.
 */
uint64_t ParaTree::getLastDescMorton(uint32_t idx) const {
 ...
 */
octantIterator ParaTree::getInternalOctantsBegin() {
 ...
 */
octantIterator ParaTree::getInternalOctantsEnd() {
 ...
 */
octantIterator ParaTree::getPboundOctantsBegin() {
 ...
 */
octantIterator ParaTree::getPboundOctantsEnd() {
    return m_pborders.end();
}

/*! Set the codimension for 2:1 balancing
 * \param[in] b21codim  Maximum codimension of the entity through which the 2:1 balance is performed (1 = 2:1 balance
 * through edges (default); 2 = 2:1 balance through nodes and edges).
 */
void ParaTree::setBalanceCodimension(uint8_t b21codim) {
    if(m_lastOp == OP_PRE_ADAPT) {
        throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
    }
 ...
 */
uint32_t ParaTree::getNumIntersections() const {
 ...
 */
Intersection* ParaTree::getIntersection(uint32_t idx) {
 ...
 */
uint8_t ParaTree::getLevel(const Intersection* inter) const {
 ...
 */
bool ParaTree::getFiner(const Intersection* inter) const {
 ...
 */
bool ParaTree::getBound(const Intersection* inter) const {
 ...
 */
bool ParaTree::getIsGhost(const Intersection* inter) const {
 ...
 */
bool ParaTree::getPbound(const Intersection* inter) const {
 ...
 */
uint8_t ParaTree::getFace(const Intersection* inter) const {
 ...
 */
u32vector ParaTree::getOwners(const Intersection* inter) const {
    u32vector owners(2);
 ...
 */
uint32_t ParaTree::getIn(const Intersection* inter) const {
    return inter->getIn();
}
 ...
 */
uint32_t ParaTree::getOut(const Intersection* inter) const {
    return inter->getOut();
}
 ...
 */
bool ParaTree::getOutIsGhost(const Intersection* inter) const {
 ...
 */
double ParaTree::getSize(const Intersection* inter) const {
    uint32_t Size;
    if(inter->m_finer && inter->m_isghost)
 ...
 */
double ParaTree::getArea(const Intersection* inter) const {
    uint64_t Area;
    if(inter->m_finer && inter->m_isghost)
 ...
 */
darray3 ParaTree::getCenter(const Intersection* inter) const {
    Octant oct(m_dim);
    if(inter->m_finer && inter->m_isghost)
 ...
 */
darr3vector ParaTree::getNodes(const Intersection* inter) const {
    darr3vector nodes;
    Octant oct(m_dim);
    if(inter->m_finer && inter->m_isghost)
        oct = m_octree.extractGhostOctant(inter->m_owners[inter->m_finer]);
 ...
 */
darray3 ParaTree::getNormal(const Intersection* inter) const {
    Octant oct(m_dim);
    if(inter->m_finer && inter->m_isghost)
        oct = m_octree.extractGhostOctant(inter->m_owners[inter->m_finer]);
    else
 ...
 */
Octant* ParaTree::getOctant(uint32_t idx) {
 ...
 */
const Octant* ParaTree::getOctant(uint32_t idx) const {
    return (m_octree.m_octants.data() + idx);
};
 ...
 */
Octant* ParaTree::getGhostOctant(uint32_t idx) {
 ...
 */
const Octant* ParaTree::getGhostOctant(uint32_t idx) const {
    return (m_octree.m_ghosts.data() + idx);
};
 ...
 */
bool ParaTree::getIsGhost(const Octant* oct) const {
 ...
 */
int ParaTree::getGhostLayer(const Octant* oct) const {
 ...
 */
const ParaTree::LoadBalanceRanges& ParaTree::getLoadBalanceRanges() const {
 ...
 */
std::size_t ParaTree::getNofGhostLayers() const {
 ...
 */
void ParaTree::setNofGhostLayers(std::size_t nofGhostLayers) {
...
/*! Set the dimension
 */
void ParaTree::setDim(uint8_t dim) {
    m_dim = dim;
    if(m_dim != 0) {
 ...
 */
void ParaTree::updateGlobalFirstDescMorton() {
    // Exchange first descendant information
    uint64_t firstDescMorton = m_octree.getFirstDescMorton();
    m_errorFlag =
        MPI_Allgather(&firstDescMorton, 1, MPI_UINT64_T, m_partitionFirstDesc.data(), 1, MPI_UINT64_T, m_comm);
 ...
 */
void ParaTree::updateGlobalLasttDescMorton() {
 ...
 * \param[in] entityCodim Codimension of the entity (1=face, 2=edge and 3=vertex for 3D trees, 1=face, 2=vertex for 2D
 * trees) \param[out] neighbours Vector with the index of the neighbours in their container \param[out] isghost Vector
 * with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be ignored in serial runs
 * \param[in] onlyinternal A boolean flag to specify if neighbours have to be found among all the octants (false) or
 * only among the internal ones (true).*/
void ParaTree::findNeighbours(const Octant* oct, uint8_t entityIdx, uint8_t entityCodim, u32vector& neighbours,
                              bvector& isghost, bool onlyinternal) const {
    if(entityCodim == 1) {
 ...
 * trees) \param[out] neighbours Vector with the index of the neighbours in their container \param[out] isghost Vector
 * with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be ignored in serial runs. */
void ParaTree::findNeighbours(uint32_t idx, uint8_t entityIdx, uint8_t entityCodim, u32vector& neighbours,
 ...
 * \param[in] entityIdx Index of face/edge/node passed through for neighbours finding
 * \param[in] entityCodim Codimension of the entity (1=face, 2=edge and 3=vertex for 3D trees, 1=face, 2=vertex for 2D
 * trees) \param[out] neighbours Vector of internal neighbours indices in octants/ghosts structure */
void ParaTree::findNeighbours(uint32_t idx, uint8_t entityIdx, uint8_t entityCodim, u32vector& neighbours) const {
    const Octant* oct = &m_octree.m_octants[idx];
    bvector isghost;
 ...
 * \param[in] entityIdx Index of face/edge/node passed through for neighbours finding
 * \param[in] entityCodim Codimension of the entity (1=face, 2=edge and 3=vertex for 3D trees, 1=face, 2=vertex for 2D
 * trees) \param[out] neighbours Vector with the index of the neighbours in their container
 */
void ParaTree::findGhostNeighbours(uint32_t idx, uint8_t entityIdx, uint8_t entityCodim, u32vector& neighbours) const {
 ...
 * with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be ignored in serial runs.
 */
void ParaTree::findGhostNeighbours(uint32_t idx, uint8_t entityIdx, uint8_t entityCodim, u32vector& neighbours,
                                   bvector& isghost) const {
    const Octant* oct = &m_octree.m_ghosts[idx];
    findNeighbours(oct, entityIdx, entityCodim, neighbours, isghost, false);
};
 ...
 * with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be ignored in serial runs.
 */
void ParaTree::findGhostNeighbours(const Octant* oct, uint8_t entityIdx, uint8_t entityCodim, u32vector& neighbours,
                                   bvector& isghost) const {
    findNeighbours(oct, entityIdx, entityCodim, neighbours, isghost, false);
};
 ...
 * \param[out] isghost Vector with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be
 * ignored in serial runs.
 */
void ParaTree::findAllNodeNeighbours(const Octant* oct, uint32_t node, u32vector& neighbours, bvector& isghost) const {
    int dim = getDim();
 ...
 * ignored in serial runs.
 */
void ParaTree::findAllNodeNeighbours(uint32_t idx, uint32_t node, u32vector& neighbours, bvector& isghost) {
 ...
 * ignored in serial runs.
 */
void ParaTree::findAllCodimensionNeighbours(uint32_t idx, u32vector& neighbours, bvector& isghost) {
    Octant* oct = getOctant(idx);
 ...
 * ignored in serial runs.
 */
void ParaTree::findAllCodimensionNeighbours(const Octant* oct, u32vector& neighbours, bvector& isghost) {
 ...
 * ignored in serial runs.
 */
void ParaTree::findGhostAllCodimensionNeighbours(uint32_t idx, u32vector& neighbours, bvector& isghost) {
 ...
 * ignored in serial runs.
 */
void ParaTree::findGhostAllCodimensionNeighbours(Octant* oct, u32vector& neighbours, bvector& isghost) {
    findAllCodimensionNeighbours(oct, neighbours, isghost);
}
 ...
 */
Octant* ParaTree::getPointOwner(const dvector& point) {
 ...
 */
Octant* ParaTree::getPointOwner(const dvector& point, bool& isghost) {
 ...
 * \return Pointer to octant owner of target point (=NULL if point is outside of the domain).
 */
Octant* ParaTree::getPointOwner(const darray3& point) {
    uint32_t idx = getPointOwnerIdx(point);
    if(idx < numeric_limits<uint32_t>::max())
        return &m_octree.m_octants[idx];
    else
 ...
 */
Octant* ParaTree::getPointOwner(const darray3& point, bool& isghost) {
    uint32_t idx = getPointOwnerIdx(point, isghost);
    if(idx < numeric_limits<uint32_t>::max())
        if(isghost)
            return &m_octree.m_ghosts[idx];
 ...
 */
uint32_t ParaTree::getPointOwnerIdx(const darray3& point) const {
 ...
 */
uint32_t ParaTree::getPointOwnerIdx(const darray3& point, bool& isghost) const {
 ...
 * \return Index of octant owner of target point (max uint32_t representable if point outside of the domain).
 */
uint32_t ParaTree::getPointOwnerIdx(const dvector& point) const {
    assert(point.size() >= 3);
    return getPointOwnerIdx(point.data());
};
 ...
 * \return Index of octant owner of target point (max uint32_t representable if point outside of the ghosted domain).
 */
uint32_t ParaTree::getPointOwnerIdx(const dvector& point, bool& isghost) const {
    assert(point.size() >= 3);
    return getPointOwnerIdx(point.data(), isghost);
};
 ...
 * \return Index of octant owner of target point (max uint32_t representable if point outside of the domain).
 */
uint32_t ParaTree::getPointOwnerIdx(const double* point) const {
    ...
    if(point[0] > 1 + m_tol || point[1] > 1 + m_tol || point[2] > 1 + m_tol || point[0] < -m_tol || point[1] < -m_tol ||
       point[2] < -m_tol) {
        return numeric_limits<uint32_t>::max();
    }
 ...
 * \return Index of octant owner of target point (max uint32_t representable if point outside of the ghosted domain).
 */
uint32_t ParaTree::getPointOwnerIdx(const double* point, bool& isghost) const {
    uint32_t noctants = m_octree.m_octants.size();
    if(noctants == 0)
        return numeric_limits<uint32_t>::max();
    uint32_t idxtry = noctants / 2;
 ...
 */
int ParaTree::getPointOwnerRank(darray3 point) {
    uint32_t x, y, z;
    uint64_t morton;
    ...
    uint32_t maxLength = getMaxLength();
    if((x > maxLength) || (y > maxLength) || (z > maxLength) || (point[0] < m_trans.m_origin[0]) ||
       (point[1] < m_trans.m_origin[1]) || (point[2] < m_trans.m_origin[2])) {
        return -1;
    }
...
/** Get the local index of the node of a target octant, corresponding to the splitting node of its family; i.e. the
 * index of the local node coincident with the center point of its father. \param[in] oct Pointer to target octant
 * \return Local index of octant node corresponding to the splitting family node.
 */
uint8_t ParaTree::getFamilySplittingNode(const Octant* oct) const {
 ...
 * \param[out] result pointer to vector of expcted octants resulting from adaptation [marker<0 : one-coarsening octant
 * father; marker=0, copy of the current input octant itself; marker>0 one-refinement octant children]
 */
void ParaTree::expectedOctantAdapt(const Octant* oct, int8_t marker, octvector* result) const {
 ...
 * \param[out] mapper Mapper from new octants to old octants. I.e. mapper[i] = j -> the i-th octant after adapt was in
 * the j-th position before adapt; if the i-th octant is new after refinement the j-th old octant was the father of the
 * new octant; if the i-th octant is new after coarsening the j-th old octant was a child of the new octant (mapper size
 * = 4). \param[out] isghost Info on ghostness of old octants. I.e. isghost[i] = true/false -> the mapper[i] = j-th old
 * octant was a local/ghost octant.
 */
void ParaTree::getMapping(uint32_t& idx, u32vector& mapper, bvector& isghost) const {
 ...
 * \param[in] idx Index of new octant.
 * \param[out] mapper Mapper from new octants to old octants. I.e. mapper[i] = j -> the i-th octant after adapt was in
 * the j-th position before adapt; if the i-th octant is new after refinement or loadbalance the j-th old octant was the
 * father of the new octant or the same octant respectively; if the i-th octant is new after coarsening the j-th old
 * octant was a child of the new octant (mapper size = 4). \param[out] isghost Info on ghostness of old octants.
 ...
 */
void ParaTree::getMapping(uint32_t& idx, u32vector& mapper, bvector& isghost, ivector& rank) const {
    if(m_lastOp == OP_ADAPT_MAPPED) {
 ...
 */
void ParaTree::getPreMapping(u32vector& idx, vector<int8_t>& markers, vector<bool>& isghost) {
    if(m_lastOp != OP_PRE_ADAPT) {
        throw std::runtime_error("Last operation different from preadapt, unable to call getPreMarker function");
    }
 ...
 * berformed
 */
bool ParaTree::isNodeOnOctant(const Octant* nodeOctant, uint8_t nodeIndex, const Octant* octant) const {
 ...
 * berformed
 */
bool ParaTree::isEdgeOnOctant(const Octant* edgeOctant, uint8_t edgeIndex, const Octant* octant) const {
 ...
 * berformed
 */
bool ParaTree::isFaceOnOctant(const Octant* faceOctant, uint8_t faceIndex, const Octant* octant) const {
    int dim = octant->getDim();
 ...
 */
bool ParaTree::adapt(bool mapper_flag) {
    bool done = false;
 ...
 */
bool ParaTree::adaptGlobalRefine(bool mapper_flag) {
    ...
    if(m_serial) {
        ...
        (*m_log) << " " << endl;
        (*m_log) << " Initial Number of octants		:	" +
                        to_string(static_cast<unsigned long long>(getNumOctants()))
                 << endl;
        ...
        if(getNumOctants() > nocts0)
            globalDone = true;
        (*m_log) << " Number of octants after Refine	:	" +
                        to_string(static_cast<unsigned long long>(getNumOctants()))
                 << endl;
        ...
        (*m_log) << " " << endl;
        (*m_log) << " Initial Number of octants		:	" +
                        to_string(static_cast<unsigned long long>(m_globalNumOctants))
                 << endl;
 ...
 * \param[in] mapper_flag True/false for tracking/not tracking the changes in structure octant .
 */
bool ParaTree::adaptGlobalCoarse(bool mapper_flag) {
    // TODO recoding for adapting with abs(marker) > 1
    uint32_t nocts0 = getNumOctants();
    vector<Octant>::iterator iter, iterend = m_octree.m_octants.end();
    ...
    if(m_serial) {
        ...
        (*m_log) << " Number of octants after Coarse	:	" + to_string(static_cast<unsigned long long>(nocts0))
                 << endl;
        (*m_log) << " " << endl;
        (*m_log) << "---------------------------------------------" << endl;
#if BITPIT_ENABLE_MPI == 1
        ...
        m_errorFlag = MPI_Allreduce(&localDone, &globalDone, 1, MPI_C_BOOL, MPI_LOR, m_comm);
        (*m_log) << " Number of octants after Coarse	:	" +
                        to_string(static_cast<unsigned long long>(m_globalNumOctants))
                 << endl;
        (*m_log) << " " << endl;
        (*m_log) << "---------------------------------------------" << endl;
 ...
 */
int8_t ParaTree::getMaxDepth() const {
    return m_maxDepth;
};
 ...
 */
int ParaTree::findOwner(uint64_t morton) const {
 ...
 * \return Rank of the process owning the element
 */
int ParaTree::getOwnerRank(uint64_t globalIndex) const {
    // Get the iterator point to the onwer rank
    std::vector<uint64_t>::const_iterator rankItr =
        std::lower_bound(m_partitionRangeGlobalIdx.begin(), m_partitionRangeGlobalIdx.end(), globalIndex);
 ...
 */
const u32vector2D& ParaTree::getConnectivity() const {
 ...
 */
const u32vector& ParaTree::getConnectivity(uint32_t idx) const {
 ...
 */
const u32vector& ParaTree::getConnectivity(Octant* oct) const {
 ...
 */
const u32arr3vector& ParaTree::getNodes() const {
 ...
 */
const u32array3& ParaTree::getNodeLogicalCoordinates(uint32_t node) const {
 ...
 */
darray3 ParaTree::getNodeCoordinates(uint32_t node) const {
 ...
 * the connectivity of each octant (4/8 indices of nodes for 2D/3D case).
 */
const u32vector2D& ParaTree::getGhostConnectivity() const {
 ...
 * (4/8 indices of nodes for 2D/3D case).
 */
const u32vector& ParaTree::getGhostConnectivity(uint32_t idx) const {
 ...
 * (4/8 indices of nodes for 2D/3D case).
 */
const u32vector& ParaTree::getGhostConnectivity(const Octant* oct) const {
            ...
            for(uint8_t f = 0; f < nCodimSubelements; ++f) {
                ...
                for(size_t n = 0; n < nsize; ++n) {
                    ...
                    if(levelDiff > 1) {
                        ...
                        (*m_log) << "I'm " << getRank() << ": element " << i << " is not 2:1 balanced across " << int(f)
                                 << " subentity of codim " << int(c) << ", relative to "
                                 << (isghost[n] ? "ghost" : "internal") << "neighbour " << neighs[n] << std::endl;
                        (*m_log) << "---------------------------------------------" << std::endl;
 ...
 */
void ParaTree::loadBalance(dvector* weight) {
    // Write info on log
    (*m_log) << "---------------------------------------------" << endl;
    ...
    if(m_nproc > 1) {
        ...
        (*m_log) << " Octants for proc	" + to_string(static_cast<unsigned long long>(0)) + "	:	" +
                        to_string(static_cast<unsigned long long>(m_partitionRangeGlobalIdx[0] + 1))
                 << endl;
        for(int ii = 1; ii < m_nproc; ii++) {
            (*m_log) << " Octants for proc	" + to_string(static_cast<unsigned long long>(ii)) + "	:	" +
                            to_string(static_cast<unsigned long long>(m_partitionRangeGlobalIdx[ii] -
                                                                      m_partitionRangeGlobalIdx[ii - 1]))
                     << endl;
        }
        (*m_log) << " " << endl;
        (*m_log) << "---------------------------------------------" << endl;
 ...
 * \param[in] level Number of level over the max depth reached in the tree at which families of octants are fixed
 * compact on the same process (level=0 is classic LoadBalance). \param[in] weight Pointer to a vector of weights of the
 * local octants (weight=NULL is uniform distribution).
 */
void ParaTree::loadBalance(uint8_t& level, dvector* weight) {
    // Write info on log
    (*m_log) << "---------------------------------------------" << endl;
    (*m_log) << " LOAD BALANCE " << endl;
 ...
 * processes.
 */
ParaTree::LoadBalanceRanges ParaTree::evalLoadBalanceRanges(dvector* weights) {
 ...
 * processes.
 */
ParaTree::LoadBalanceRanges ParaTree::evalLoadBalanceRanges(uint8_t level, dvector* weights) {
 ...
 * processes.
 */
ParaTree::LoadBalanceRanges ParaTree::evalLoadBalanceRanges(const uint32_t* updatedPartition) {
    // If there is only one process no octants can be sent
    if(m_nproc == 1) {
        LoadBalanceRanges loadBalanceInfo;
        loadBalanceInfo.sendAction = LoadBalanceRanges::ACTION_NONE;
 ...
 */
ParaTree::ExchangeRanges ParaTree::evalLoadBalanceSendRanges(const uint32_t* updatedPartition) {
    ...
    PartitionIntersections globalIntersections =
        evalPartitionIntersections(currentPartition.data(), m_rank, updatedPartition);
 ...
 * processes.
 */
ParaTree::ExchangeRanges ParaTree::evalLoadBalanceRecvRanges(const uint32_t* updatedPartition) {
    ...
    // Get the intersections
    PartitionIntersections globalIntersections =
        evalPartitionIntersections(updatedPartition, m_rank, currentPartition.data());
 ...
 * partition schema B.
 */
ParaTree::PartitionIntersections ParaTree::evalPartitionIntersections(const uint32_t* partition_A, int rank_A,
 ...
 */
double ParaTree::levelToSize(uint8_t level) const {
 ...
 */
void ParaTree::computeIntersections() {
 ...
 */
Octant& ParaTree::extractOctant(uint32_t idx) {
 ...
 */
bool ParaTree::private_adapt_mapidx(bool mapflag) {
    // TODO recoding for adapting with abs(marker) > 1
 ...
 */
void ParaTree::computePartition(uint32_t* partition) {
 ...
 */
void ParaTree::computePartition(const dvector* weight, uint32_t* partition) {
 ...
 */
void ParaTree::computePartition(uint8_t level_, const dvector* weight, uint32_t* partition) {
    ...
    m_errorFlag =
        MPI_Allgather(&new_interfaces_count, 1, MPI_UINT8_T, new_interfaces_count_per_rank, 1, MPI_UINT8_T, m_comm);
    m_errorFlag = MPI_Allgather(&first_new_interface_rank_index, 1, MPI_UINT8_T,
                                first_new_interface_rank_index_per_rank, 1, MPI_UINT8_T, m_comm);
    ...
    partition_temp = NULL;
    delete[] new_boundary_owner;
    new_boundary_owner = NULL;
    delete[] new_interfaces_count_per_rank;
    new_interfaces_count_per_rank = NULL;
    delete[] first_new_interface_rank_index_per_rank;
    first_new_interface_rank_index_per_rank = NULL;
    delete[] deplace;
    deplace = NULL;
}
 ...
 */
void ParaTree::updateLoadBalance() {
    // update sizes
...
void ParaTree::setPboundGhosts() {
    ...
    for(uint32_t idx = 0; idx < m_octree.getNumOctants(); ++idx) {
        ...
        for(uint8_t e = 0; e < m_treeConstants->nEdges; ++e) {
            octant.computeEdgeVirtualMortons(e, m_maxDepth, m_octree.m_balanceCodim, m_treeConstants->edgeFace,
                                             &nVirtualNeighbors, &virtualNeighbors);
            if(nVirtualNeighbors > 0) {
                uint32_t maxDelta = nVirtualNeighbors / 2;
        ...
        for(uint8_t c = 0; c < m_treeConstants->nNodes; ++c) {
            ...
            octant.computeNodeVirtualMorton(c, m_maxDepth, m_treeConstants->nodeFace, &hasVirtualNeighbour,
                                            &virtualNeighbor);
...
/*! Build the structure with the information about the layers (from the second one) of ghost octants, partition boundary
 * octants and parameters for communicate between processes.
 */
void ParaTree::computeGhostHalo() {
 ...
 */
void ParaTree::initializeGhostHaloAccretions(std::vector<AccretionData>* accretions) {
 ...
 * \param[in,out] accretions list of accretions
 */
void ParaTree::growGhostHaloAccretions(std::unordered_map<uint32_t, std::vector<uint64_t>>* oneRingsCache,
                                       std::vector<AccretionData>* accretions) {
 ...
 * \param[in,out] accretions list of accretions
 */
void ParaTree::exchangeGhostHaloAccretions(DataCommunicator* dataCommunicator, std::vector<AccretionData>* accretions) {
 ...
 */
void ParaTree::buildGhostOctants(const std::map<int, u32vector>& bordersPerProc,
...
void ParaTree::commMarker() {
    ...
    // for every element it visits the border octants it contains and write them in the bitpit communication structure,
    // DataCommunicator this structure has a buffer for every proc containing the octants to be sent to that proc
    // written in a char* buffer
 ...
 */
void ParaTree::updateAfterCoarse() {
    updateAdapt();
 ...
 * balanced
 */
void ParaTree::balance21(bool verbose, bool balanceNewOctants) {
    // Print header
    if(verbose) {
        (*m_log) << "---------------------------------------------" << endl;
 ...
 * \param[in] filename Name of output file (PABLO will add the total number of processes p000# and the current rank
 * s000#).
 */
void ParaTree::write(const std::string& filename) {
    if(m_octree.m_connectivity.size() == 0) {
        m_octree.computeConnectivity();
    }

    stringstream name;
    name << "s" << std::setfill('0') << std::setw(4) << m_nproc << "-p" << std::setfill('0') << std::setw(4) << m_rank
         << "-" << filename << ".vtu";
    ...
    out << "<?xml version=\"1.0\"?>" << endl
        ...
        << "\" NumberOfPoints=\"" << m_octree.m_nodes.size() << "\">" << endl;
    out << "      <Points>" << endl
    ...
    for(int i = 0; i < nofNodes; i++) {
        for(int j = 0; j < 3; ++j) {
            if(j == 0)
                out << std::setprecision(6) << m_trans.mapX(m_octree.m_nodes[i][j]) << " ";
            if(j == 1)
                out << std::setprecision(6) << m_trans.mapY(m_octree.m_nodes[i][j]) << " ";
            if(j == 2)
                out << std::setprecision(6) << m_trans.mapZ(m_octree.m_nodes[i][j]) << " ";
        }
    ...
    if(m_rank == 0) {
        ...
        pout << "    </PCellData>" << endl
             ...
             << "    </PPoints>" << endl;
        for(int i = 0; i < m_nproc; i++)
            pout << "    <Piece Source=\"s" << std::setw(4) << std::setfill('0') << m_nproc << "-p" << std::setw(4)
                 << std::setfill('0') << i << "-" << filename << ".vtu\"/>" << endl;
        pout << "  </PUnstructuredGrid>" << endl << "</VTKFile>";
 ...
 * \param[in] filename Name of output file (PABLO will add the total number of processes p000# and the current rank
 * s000#). \param[in] data Vector of double with user data.
 */
void ParaTree::writeTest(const std::string& filename, vector<double> data) {
    if(m_octree.m_connectivity.size() == 0) {
        m_octree.computeConnectivity();
    }
    ...
    out << "<?xml version=\"1.0\"?>" << endl
        << "<VTKFile type=\"UnstructuredGrid\" version=\"0.1\" byte_order=\"BigEndian\">" << endl
        << "  <UnstructuredGrid>" << endl
        << "    <Piece NumberOfCells=\"" << m_octree.m_connectivity.size() << "\" NumberOfPoints=\""
        << m_octree.m_nodes.size() << "\">" << endl;
