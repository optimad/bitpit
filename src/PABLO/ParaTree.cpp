...
namespace bitpit {
    ...
    */
    ...
    ParaTree::getPointOwnerIdx(const double * point) const {
        ...
        //
        // If the owner of the point is an internal octant, decrementing the the Morton upper
        // bound by one will give us the requested octant.
    ...
    ParaTree::getPointOwnerIdx(const double * point, bool & isghost) const {
        ...
        //
        // If the owner of the point is an internal octant, decrementing the the Morton upper
        // bound by one will give us the requested octant. If the owner of the point is a ghost
        // octant, we need to test all the octants before the Morton upper bound.
        uint32_t pointOwnerIdx;
        uint64_t pointOwnerMorton;
        if (!isghost) {
            m_octree.findMortonUpperBound(morton, m_octree.m_octants, &pointOwnerIdx, &pointOwnerMorton);
            ...
            while (pointOwnerIdx > 0) {
                ...
                darray3 octantOrigin = getCoordinates(octant);
                double octantSize = getSize(octant);

                bool found = true;
                for (int i = 0; i < m_dim; ++i) {
                    if (point[i] < octantOrigin[i] - m_tol || point[i] > (octantOrigin[i] + octantSize + m_tol)) {
                        found = false;
                        break;
                    }
                }
