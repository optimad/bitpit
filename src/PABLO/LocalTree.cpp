...
//                            }
    ...
    LocalTree::findNeighbours(const Octant* oct, uint8_t iface, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
        ...
        u32vector neighbours_OLD;
        bvector isghost_OLD;
        LocalTree::findNeighbours_OLD(oct, false, 0, iface, neighbours_OLD, isghost_OLD, onlyinternal);

        u32vector neighbours_NEW;
        bvector isghost_NEW;
        LocalTree::findNeighbours_NEW(oct, iface, neighbours_NEW, isghost_NEW, onlyinternal);


        bool error = (neighbours_OLD.size() != neighbours_NEW.size());
        if (!error) {
            int nNeighs = neighbours_OLD.size();
            for (int i = 0; i < nNeighs; ++i) {
                if (neighbours_OLD[i] != neighbours_NEW[i]) {
                    error = true;
                    break;
                }
            }
        ...
        if (error) {
            log::cout() << " NEGIH ERROR BEGIN " << std::endl;
            log::cout() << " octant  is ghost: " << oct->getIsGhost() << std::endl;
            if (!oct->getIsGhost()) {
                log::cout() << " octant index: " << (m_octants.data() - oct) << std::endl;
            } else {
                log::cout() << " octant index: " << (m_ghosts.data() - oct) << std::endl;
            }
            log::cout() << " face: " << (int) iface << std::endl;
            for (uint32_t id : neighbours_OLD) {
                log::cout() << " OLD " << id << std::endl;
            }

            for (uint32_t id : neighbours_OLD) {
                log::cout() << " NEW " << id << std::endl;
            }
            log::cout() << " NEGIH ERROR END " << std::endl;
        }

        neighbours = neighbours_OLD;
        isghost = isghost_OLD;
     ...
     */
    void
    LocalTree::findNeighbours_OLD(const Octant* oct, bool haveIidx, uint32_t idx, uint8_t iface, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
     ...
     */
    void
    LocalTree::findNeighbours_NEW(const Octant* oct, uint8_t iface, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
        ...
        // Search same size neighbour
        _findLastLowerEqualMorton(samesizemorton, m_octants, &idxtry, &mortontry);

        // Early return if a neighbour of the same size has been found
        if(mortontry == samesizemorton && m_octants[idxtry].m_level == level){
            neighbours.push_back(idxtry);
            isghost.push_back(false);
            return;
        }
        ...
        if (idxtry < m_sizeOctants){
            while(true){
                ...
                for (int idim=0; idim<m_dim; idim++){
                    ...
                    int32_t Dxstar = int32_t((cxyz[idim]-1)/2)*(m_octants[idxtry].getLogicalSize()) + int32_t((cxyz[idim]+1)/2)*size;
                    if (Dx != Dxstar){
                ...
                if (isNeighbourCandidate){
                    ...
                    if (isNeighbour){
                        ...
                        neighArea += m_octants[idxtry].getLogicalArea();
                        if (neighArea == oct->getLogicalArea()){
                            return;
                        }
                    }
                ...
                idxtry++;
                if (idxtry > m_sizeOctants-1){
                    break;
                }

                mortontry = m_octants[idxtry].computeMorton();
                if (mortontry > mortonlast){
                    break;
                }
            }
        }
        ...
        if(ghostSearch){
            // Search same size neighbour
            _findLastLowerEqualMorton(samesizemorton, m_ghosts, &idxtry, &mortontry);

            // Early return if a neighbour of the same size has been found
            if(mortontry == samesizemorton && m_ghosts[idxtry].getLevel() == level){
                neighbours.push_back(idxtry);
                isghost.push_back(true);
                return;
            }
            ...
            if (idxtry < m_sizeGhosts){
                while(true){
                    ...
                    for (int idim=0; idim<m_dim; idim++){
                        int32_t Dx     = int32_t(int32_t(abs(cxyz[idim]))*(-coord[idim] + coordtry[idim]));
                        int32_t Dxstar = int32_t((cxyz[idim]-1)/2)*(m_ghosts[idxtry].getLogicalSize()) + int32_t((cxyz[idim]+1)/2)*size;
                        if (Dx != Dxstar){
                            isNeighbourCandidate = false;
                            break;
                        }
                    ...
                    if (isNeighbourCandidate){
                        ...
                        if (isNeighbour){
                            ...
                            // If the neighbours already cover the whole face, we have
                            // found all the neighbours and we can exit.
                            neighArea += m_ghosts[idxtry].getLogicalArea();
                            if (neighArea == oct->getLogicalArea()){
                                return;
                            }
                        }
