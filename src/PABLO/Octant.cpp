 ...
 * \param[in] bound Boundary condition for the faces of the octant (the same for each face).
 */
void Octant::initialize(uint8_t dim, uint8_t level, bool bound)
{
    m_dim   = dim;
    m_level = level;
    ...
    for (uint8_t i = INFO_ITEM_COUNT + 1; i < EXT_INFO_ITEM_COUNT; ++i) {
        m_info[i] = 0;
    }
};
 ...
 */
bool Octant::getBound(uint8_t face) const
{
 ...
 */
bool Octant::getEdgeBound(uint8_t edge) const
{
 ...
 */
bool Octant::getNodeBound(uint8_t node) const
{
 ...
 */
void Octant::setBound(uint8_t face)
{
 ...
 */
bool Octant::getPbound(uint8_t face) const
{
 ...
 */
bool Octant::getIsNewR() const { return m_info[OctantInfo::INFO_NEW4REFINEMENT]; };

/*! Get if the octant is new after a coarsening.
 * \return true if the the octant is new after a coarsening.
 */
bool Octant::getIsNewC() const { return m_info[OctantInfo::INFO_NEW4COARSENING]; };

/*! Get if the octant is a scary ghost octant.
 * \return true if the octant is a ghost octant.
 */
bool Octant::getIsGhost() const { return (m_ghost >= 0); };
 ...
 */
int Octant::getGhostLayer() const { return m_ghost; };

/*! Get if the octant has to be balanced.
 * \return true if the octant has to be balanced.
 */
bool Octant::getBalance() const { return (m_info[OctantInfo::INFO_BALANCED]); };
 ...
 */
void Octant::setMarker(int8_t marker)
{
 ...
 */
void Octant::setBalance(bool balance)
{
 ...
 */
void Octant::setLevel(uint8_t level)
{
    this->m_level = level;
};
 ...
 */
void Octant::setPbound(uint8_t face, bool flag)
{
 ...
 */
void Octant::setGhostLayer(int ghostLayer)
{
    m_ghost = ghostLayer;
};

bool Octant::getExtInfo(uint8_t item)
{
    return m_info[item + INFO_ITEM_COUNT];
};

void Octant::setExtInfo(uint8_t item, bool flag)
{
 ...
 */
void Octant::getLogicalNodes(u32arr3vector &nodes) const
{
    uint8_t i;
    uint32_t dh;
