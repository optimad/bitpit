...
{
    m_energyLevel = energy - ENERGY_CHECK_TOLERANCE;
}
...
{
    return m_energyLevel + ENERGY_CHECK_TOLERANCE;
}
 ...
 *
 * \param[in/out] POD Field object.
 * \param[out] matrix of size N_fields x N_modes with the coefficients of the L2 projection.
 */
std::vector<std::vector<double>> POD::projectField(pod::PODField &field)
{
    std::vector<std::vector<double>> reconstructionCoeffs(m_nFields, std::vector<double>(m_nModes,0.0));
    if (m_useMean) {
        diff(&field, m_mean);
    }

    // Outer cycle over the POD modes
    for (std::size_t ir = 0; ir < m_nModes; ++ir) {
        if (m_memoryMode == MemoryMode::MEMORY_LIGHT) {
            readMode(ir);
        }

        // Outer cycle over active cells
        for (long id : m_listActiveIDs) {
            std::size_t rawIndex = field.mesh->getCells().getRawIndex(id);

            // Inner cycle over scalar fields
            if (m_nScalarFields) {
                const double *modes = m_modes[ir].scalar->rawData(rawIndex);
                const double *data  = field.scalar->rawData(rawIndex);
                for (std::size_t ifs = 0; ifs < m_nScalarFields; ++ifs) {
                    reconstructionCoeffs[ifs][ir] += data[ifs] * modes[ifs] * getRawCellVolume(rawIndex);
                }
            }
            ...
            if (m_nVectorFields) {
                ...
                const std::array<double,3> *data = field.vector->rawData(rawIndex);
                for (std::size_t ifv = 0; ifv < m_nVectorFields; ++ifv) {
                    reconstructionCoeffs[ifv+m_nScalarFields][ir] += dotProduct(data[ifv], modes[ifv]) * getRawCellVolume(rawIndex);
                }
            }
        }
    }
...
#if BITPIT_ENABLE_MPI
    for (std::size_t i = 0; i < m_nFields; ++i) {
        MPI_Allreduce(MPI_IN_PLACE, reconstructionCoeffs[i].data(), m_nModes, MPI_DOUBLE, MPI_SUM, m_communicator);
    }
#endif
