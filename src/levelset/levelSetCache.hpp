...
public:
    ...
    virtual void writeBuffer(const std::vector<key_t> &keys, SendBuffer &buffer) = 0;
...
public:
    ...
    void writeBuffer(const std::vector<key_t> &ids, SendBuffer &buffer) override;
...
public:
    ...
    void erase(const key_t &key) override;
...
public:
    ...
    LevelSetContainerCache(PiercedKernel<key_t> *m_kernel, PiercedSyncMaster::SyncMode syncMode);
...
public:
    ...
    LevelSetContainerCacheFactory(PiercedKernel<key_t> *kernel, PiercedSyncMaster::SyncMode syncMode);

    std::unique_ptr<LevelSetCache<key_t>> create() const override;

protected:
    PiercedKernel<key_t> *m_kernel;
...
public:
    typedef std::vector<std::shared_ptr<LevelSetCacheFactory<key_t>>> Factories;
    typedef std::vector<std::unique_ptr<LevelSetCache<key_t>>> Caches;
    ...
    LevelSetCache<key_t> * at(std::size_t index);
    const LevelSetCache<key_t> * at(std::size_t index) const;

    template<typename value_t>
    LevelSetValueCache<key_t, value_t> * at(std::size_t index);
    template<typename value_t>
    const LevelSetValueCache<key_t, value_t> * at(std::size_t index) const;
...
protected:
    Factories m_factories; //!< Factories that will create the caches
    mutable Caches m_caches; //!< Caches owned by the collection
