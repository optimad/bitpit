...
std::set<long> bitpit::LevelSetSegmentation::NULL_LIST ; 
long bitpit::LevelSetSegmentation::NULL_ELEMENT=-1 ; 

/*!
 * Destructor
 */
bitpit::LSObject::~LSObject( ){
};
 ...
 */
bitpit::LSObject::LSObject( int id) : m_id(id){
};
 ...
 */
int bitpit::LSObject::getId( ) const {
    return m_id ;
};
 ...
 */
bitpit::LevelSetSegmentation::SegData::SegData( ) : m_segments(NULL_LIST), m_support(NULL_ELEMENT){
};
 ...
 * @param[in] list list of simplices
 */
bitpit::LevelSetSegmentation::SegData::SegData( const std::set<long> &list) :m_segments(list), m_support(NULL_ELEMENT) {
};
 ...
 */
bitpit::LevelSetSegmentation::SegData::SegData( const std::set<long> &list, const long &support) :m_segments(list), m_support(support){
};

/*!
 * Destructor of LevelSet_Stl.
 */
bitpit::LevelSetSegmentation::~LevelSetSegmentation() {
    stl = NULL;
};
 ...
 */
bitpit::LevelSetSegmentation::LevelSetSegmentation( int id, bitpit::SurfUnstructured *STL) :LSObject(id) {
 ...
 */
bitpit::LevelSetSegmentation::LevelSetSegmentation( const bitpit::LevelSetSegmentation &other) :LSObject(other.getId() ) {
 ...
 */
bitpit::LevelSetSegmentation* bitpit::LevelSetSegmentation::clone() const { 
 ...
 */
const std::set<long> & bitpit::LevelSetSegmentation::getSimplexList(const long &i){
 ...
 */
const long & bitpit::LevelSetSegmentation::getSupportSimplex(const long &i){
 ...
 * bitpit::LevelSetSegmentation::associateSimplexToCell() should have been called prior;
 * @param[in] i Local index of target octant.
 * @return if in narow band
 */
bool bitpit::LevelSetSegmentation::isInNarrowBand(const long &i){
    return( m_segInfo.exists(i) ) ; 
};

/*!
 * Add a bitpit::SurfTriPatch object to stl.
 * @param[in] *STL Pointer to surface Triangulation to be added .
 */
std::vector<std::array<double,3>> bitpit::LevelSetSegmentation::getSimplexVertices( const long &i ) const {

    bitpit::Cell &cell = stl->getCell(i) ;
 ...
 */
void bitpit::LevelSetSegmentation::lsFromSimplex( bitpit::LevelSet *visitee, const double &search, bool filter){
    ...
    bitpit::PiercedIterator<SegData>    segIt, segEnd = m_segInfo.end() ;
 ...
 */
void bitpit::LevelSetSegmentation::infoFromSimplex( const std::array<double,3> &p,const  long &i, double &d, double &s, std::array<double,3> &x, std::array<double,3> &n ) const {
 ...
 */
bool bitpit::LevelSetSegmentation::seedNarrowBand( bitpit::LevelSetCartesian *visitee, std::vector<std::array<double,3>> &VS, std::vector<int> &I){ 

    bitpit::VolumeKernel                 &mesh = *(visitee->m_mesh) ;
 ...
 */
void bitpit::LevelSetSegmentation::seedSign( bitpit::LevelSet *visitee, long &seed, double &value) const {
 ...
 */
void bitpit::LevelSetSegmentation::getBoundingBox( std::array<double,3> &minP, std::array<double,3> &maxP ) const {
 ...
 */
void bitpit::LevelSetSegmentation::computeLSInNarrowBand( bitpit::LevelSetCartesian *visitee ){
 ...
 */
void bitpit::LevelSetSegmentation::associateSimplexToCell( bitpit::LevelSetCartesian *visitee ){
    ...
    bitpit::PiercedVector<SegData>::iterator data ;
 ...
 * of at least 2 mesh cell centers around the geometry. 
 */
void bitpit::LevelSetSegmentation::updateLSInNarrowBand( bitpit::LevelSetCartesian *visitee, std::vector<bitpit::Adaption::Info> &mapper, double &newRSearch ){
 ...
 */
void bitpit::LevelSetSegmentation::updateSimplexToCell( bitpit::LevelSetCartesian *visitee, std::vector<bitpit::Adaption::Info> &mapper, double &newRSearch ){
 ...
 * of at least 2 mesh cell centers around the geometry. 
 */
void bitpit::LevelSetSegmentation::computeLSInNarrowBand( bitpit::LevelSetOctree *visitee ){
 ...
 */
void bitpit::LevelSetSegmentation::associateSimplexToCell( bitpit::LevelSetOctree *visitee){
    ...
    if( CGElem::intersectBoxBox(octrBB0,octrBB1,G0,G1,C0,C1) ) { //intersect two Bounding Boxes around geometry and local grid
        ...
        bitpit::VolCartesian            cmesh( 0, dim, C0, C1-C0, nc) ;

        bitpit::LevelSetCartesian       auxLS(cmesh) ; 
        bitpit::LevelSetSegmentation    objLS(*this) ; 
 ...
 * of at least 2 mesh cell centers around the geometry. 
 */
void bitpit::LevelSetSegmentation::updateLSInNarrowBand( bitpit::LevelSetOctree *visitee, std::vector<bitpit::Adaption::Info> &mapper, double &newRSearch ){
 ...
 * Note: Only a single octree adapt with marker (-1,0,1) is permitted.
 */
void bitpit::LevelSetSegmentation::updateSimplexToCell( bitpit::LevelSetOctree *visitee, std::vector<bitpit::Adaption::Info> &mapper, double &newRSearch){
    ...
    if( newLevel <= oldLevel ) { //size of narrow band decreased or remained the same -> mapping
        ...
        for ( auto & info : mapper ){ //forall mesh modifications
            if( info.entity == bitpit::Adaption::Entity::ENTITY_CELL){ //check if changes on cells
                for ( auto & child : info.current){ // forall new elements

                    bitpit::PiercedVector<SegData>::iterator seg =  m_segInfo.reclaim(child) ;
