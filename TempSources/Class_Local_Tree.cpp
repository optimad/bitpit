...
void Class_Local_Tree::checkCoarse(uint64_t lastDescPre,
...
void Class_Local_Tree::computeghostsConnectivity() {
	...
	if (nodes_ghosts.size() == 0){
		connectivity_ghosts.resize(noctants);
		for (i = 0; i < noctants; i++){
		...
		iterend	= mapnodes.end();
		uint32_t numnodes = mapnodes.size();
		nodes_ghosts.resize(numnodes);
		counter = 0;
		vector<uint32_t>::iterator iter2;
		while (iter != iterend){
			vector<uint32_t> nodecasting(iter->second.begin(), iter->second.begin()+DIM);
			nodes_ghosts[counter] = nodecasting;
			nodes_ghosts[counter].shrink_to_fit();
			for(iter2 = iter->second.begin()+DIM; iter2 != iter->second.end(); iter2++){
				if (connectivity_ghosts[(*iter2)].size()==0){
					connectivity_ghosts[(*iter2)].reserve(8);
				}
				connectivity_ghosts[(*iter2)].push_back(counter);
			}
			mapnodes.erase(iter++);
			counter++;
		}
		nodes_ghosts.shrink_to_fit();
		//Lento. Solo per risparmiare memoria
		for (int ii=0; ii<noctants; ii++){
			connectivity_ghosts[ii].shrink_to_fit();
		}
		connectivity_ghosts.shrink_to_fit();
	}
	iter = mapnodes.end();
}

void Class_Local_Tree::clearghostsConnectivity() {
	u32vector2D().swap(nodes_ghosts);
	u32vector2D().swap(connectivity_ghosts);
}
...
void Class_Local_Tree::computeIntersections() {
	...
	intersections_int.reserve(2*DIM*octants.size());
	intersections_ghost.reserve(2*DIM*ghosts.size());
	...
	for (it = obegin; it != oend; it++){
		for (iface = 0; iface < DIM; iface++){
			...
			for (i = 0; i < nsize; i++){
				...
				intersection.iface = oppface[iface2];
...
void Class_Local_Tree::updateIntersections(u32vector & mapidx,
		...
		for (it = obegin; it != oend; it++){
			for (iface = 0; iface < 2*DIM; iface++){
				...
				for (i = 0; i < nsize; i++){
					...
					intersection.owners[0]  = neighbours[i];
					intersection.owners[1] = idx;
					intersection.iface = oppface[iface];
